...
// this is simple php webhook for wmdb, not recommended using thi procedural pattern if you have a lot of keywords!
...
// respon text
if ($message == 'hello') {
    $respon = $responFormatter->line('Hi too {name}')->line('Anything I can help?')->responAsText();
}
// respon text with quoted
if ($message == 'hi') {
    $respon = $responFormatter->quoted()->line('Hi too')->line('Anything I can help?')->responAsText();
}
...
// respon button
if ($message == 'button') {
    $respon = $responFormatter->line('ini isi pesannya')->bold("bold")
        ->addButton("First button")
        ->addButton("Second button")
...
if ($message == 'button image') {
    $respon = $responFormatter->line('this is the message content')->bold("bold")
        ->addButton("First button")
        ->footer('this is the footer')
...
if ($message == 'template button') {
    $respon = $responFormatter->line('This is the content of the message.')->bold("bold")
        ->addTemplateButton("First button (visit url when clicked)"https://google.com")
        ->addTemplateButton("Second button (phone call when clicked)", "254722XXXXXX", "call")
        ->footer('this is the footer')
